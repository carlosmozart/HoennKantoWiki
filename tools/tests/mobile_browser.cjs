...
(async()=>{
  ...
  try {
    ...
    fs.writeFileSync(worker,fs.readFileSync(worker,'utf8').replace('pokewiki-v19','pokewiki-test-next'));
    await page.evaluate(async()=>{await (await navigator.serviceWorker.getRegistration()).update();});
    await page.waitForFunction(async()=>{
      const keys=await caches.keys();
      return keys.includes('pokewiki-test-next')&&!keys.includes('pokewiki-obsolete')&&!keys.includes('pokewiki-v19');
    });
    ...
    console.log(JSON.stringify({ok:true,checks:['TM/tutor translated type pills and spacing','mobile history back/forward','local cry playback','decode Charmeleon/Pikachu/Deoxys','offline reopen and audio','390px layout','SW update preserves unrelated caches','incomplete update rejected'],decoded},null,2));
