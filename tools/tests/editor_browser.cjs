...
(async()=>{
  ...
  try{
    ...
    await page.locator('#preview-version').selectOption('emerald');
    await page.waitForFunction(()=>document.querySelector('#status').textContent.includes('Prévia atualizada'));
    await page.getByLabel('Título',{exact:true}).fill('Safari revisado no editor');
    const versionedContent=page.locator('#form > details').filter({has:page.locator(':scope > summary',{hasText:'Conteúdo por jogo'})});
    await versionedContent.locator(':scope > summary').click();
    const emeraldContent=versionedContent.getByRole('textbox',{name:'Emerald',exact:true}).locator('..');
    await emeraldContent.getByRole('button',{name:'+ Card',exact:true}).click();
    await emeraldContent.locator('.rich .bento-item').last().locator('h4').fill('Card de teste Safari');
    await refresh();
    await page.frameLocator('#preview-frame').getByText('Card de teste Safari',{exact:true}).waitFor();
    await emeraldContent.locator('.rich .bento-item').last().locator('p').click();
    await emeraldContent.getByRole('button',{name:'Remover bloco',exact:true}).click();
    await refresh();
    ...
    await emeraldContent.getByText('HTML avançado',{exact:true}).click();
    await emeraldContent.getByLabel('HTML do conteúdo',{exact:true}).fill('<p>Rascunho imediato</p>');
    assert.equal(await warnsOnExit(),true);
    ...
    assert.equal(stored[0].data.safari.contentByVersion.emerald,'<p>Rascunho imediato</p>');
    await page.locator('#undo').click();
    ...
    const tabsSection=page.locator('#form fieldset').filter({has:page.locator('.array-header > label',{hasText:'Abas'})}).first();
    assert.equal(await tabsSection.locator(':scope > details').count(),1);
    await tabsSection.locator(':scope > details > summary').first().click();
    await tabsSection.getByRole('button',{name:'Adicionar de modelo',exact:true}).click();
    await page.locator('#template-list').getByRole('button',{name:'Pokémon em destaque',exact:true}).click();
    await tabsSection.locator(':scope > details > summary').first().click();
    const cardSection=tabsSection.locator(':scope > details > .fields > fieldset').filter({has:page.locator('.array-header > label',{hasText:'Cards'})}).first();
    ...
    await page.locator('#workspace-media').click();
    await page.locator('#media-results .media-card').first().waitFor();
    await page.locator('#media-results summary').first().click();
    assert.ok((await page.locator('#media-results').innerText()).includes('Rascunho: pages.json'));
    await page.locator('#media-filter').selectOption('unreferenced');
    assert.equal(await page.locator('#media-results .media-card').count(),0);
    await page.locator('#media-filter').selectOption('all');
    await page.locator('#media-search').fill('nova-imagem');
    assert.equal(await page.locator('#media-results .media-card').count(),1);
    await page.setViewportSize({width:390,height:844});
    assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    await page.screenshot({path:path.join(out,'editor-media-mobile.png'),animations:'disabled'});
    await page.locator('#close-workspace').click();
    await page.setViewportSize({width:1440,height:1000});
    await page.locator('#workspace-sync').click();
    await page.waitForFunction(()=>document.querySelector('#sync-status').textContent.includes('Git indisponível'));
    await page.locator('#close-workspace').click();
    reports.push('biblioteca pesquisa imagens e inclui referencias de rascunhos; status sem Git tratado');
    await tabsSection.locator(':scope > .array-header').getByRole('button',{name:'+ Adicionar',exact:true}).click();
    assert.equal(await tabsSection.locator(':scope > details').count(),2);
    const secondTab=tabsSection.locator(':scope > details').nth(1);
    await secondTab.locator(':scope > summary').click();
    await secondTab.getByLabel('Identificador da aba',{exact:true}).fill('dicas');
    await secondTab.getByLabel('Rótulo',{exact:true}).first().fill('Dicas');
    await secondTab.getByRole('button',{name:'Adicionar de modelo',exact:true}).click();
    await page.locator('#template-list').getByRole('button',{name:'Pokémon em destaque',exact:true}).click();
    await refresh();
    ...
    assert.equal(await page.frameLocator('#preview-frame').locator('.custom-page-tab').count(),2);
    await page.frameLocator('#preview-frame').getByRole('link',{name:'Dicas',exact:true}).click();
    ...
    await page.locator('#section').selectOption({label:'Menus personalizados'});
    await page.locator('#add').click();
    await page.getByLabel('Identificador do menu',{exact:true}).first().fill('menu-teste');
    await page.getByLabel('Rótulo',{exact:true}).first().fill('Menu de teste');
    let menuEntries=page.locator('#form fieldset').filter({has:page.locator('.array-header > label',{hasText:'Itens do menu'})}).first();
    await menuEntries.locator(':scope > .array-header').getByRole('button',{name:'+ Adicionar',exact:true}).click();
    menuEntries=page.locator('#form fieldset').filter({has:page.locator('.array-header > label',{hasText:'Itens do menu'})}).first();
    await menuEntries.locator(':scope > details > summary').first().click();
    await menuEntries.getByLabel('Identificador do menu',{exact:true}).first().fill('guias-extras');
    await menuEntries.getByLabel('Rótulo',{exact:true}).first().fill('Guias extras');
    let submenu=menuEntries.locator(':scope > details > .fields > fieldset').filter({has:page.locator('.array-header > label',{hasText:'Submenus'})}).first();
    await submenu.locator(':scope > .array-header').getByRole('button',{name:'+ Adicionar',exact:true}).click();
    menuEntries=page.locator('#form fieldset').filter({has:page.locator('.array-header > label',{hasText:'Itens do menu'})}).first();
    await menuEntries.locator(':scope > details > summary').first().click();
    submenu=menuEntries.locator(':scope > details > .fields > fieldset').filter({has:page.locator('.array-header > label',{hasText:'Submenus'})}).first();
    await submenu.locator(':scope > details > summary').first().click();
    await submenu.getByLabel('Identificador do menu',{exact:true}).first().fill('guia-personalizado');
    await submenu.getByLabel('Rótulo',{exact:true}).first().fill('Guia vinculado');
    await submenu.getByLabel('Página vinculada',{exact:true}).first().fill('guia-personalizado');
    await refresh();
    await page.frameLocator('#preview-frame').getByText('Menu de teste',{exact:true}).click();
    await page.frameLocator('#preview-frame').getByText('Guias extras',{exact:true}).click();
    await page.frameLocator('#preview-frame').getByRole('link',{name:'Guia vinculado',exact:true}).waitFor();
    await page.locator('#section').selectOption({label:'Páginas'});
    await page.locator('#entries').getByRole('button',{name:'Meu guia personalizado',exact:true}).click();
    await page.locator('#save').click();
    await page.locator('#confirm-review').click();
    await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Salvo no projeto.'));
    await refresh();
    ...
    await page.locator('#workspace-media').click();
    await page.waitForFunction(()=>!document.querySelector('#media-refresh').disabled);
    await page.locator('#media-filter').selectOption('uploads');
    await page.locator('#media-search').fill('');
    await page.locator('#media-results summary').first().click();
    assert.ok((await page.locator('#media-results').innerText()).includes('data/pages.json'));
    await page.locator('#close-workspace').click();
    reports.push('imagem salva mostra o documento e o campo onde aparece');
