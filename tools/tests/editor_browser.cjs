...
(async()=>{
  ...
  try{
    ...
    // A restored backup is a draft until reviewed and saved; the current version remains recoverable.
    await page.locator('#backups').click();
    await page.locator('#backup-list button').first().click();
    await page.locator('#review-dialog').waitFor();
    assert.ok((await page.locator('#review-list').innerText()).includes('Categoria de teste'));
    await page.locator('#cancel-review').click();
    assert.equal(await page.getByLabel('Categoria',{exact:true}).inputValue(),'Categoria de teste');
    await page.locator('#backup-list button').first().click();
    await page.locator('#confirm-review').click();
    await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Backup carregado'));
    assert.equal(await page.getByLabel('Categoria',{exact:true}).inputValue(),before.hoenn[0].category);
    assert.equal(JSON.parse(fs.readFileSync(path.join(config.root,'data/key-items.json'))).hoenn[0].category,'Categoria de teste');
    ...
    await page.screenshot({path:path.join(out,'editor-review-mobile.png'),animations:'disabled'});
    await page.locator('#confirm-review').click();
    await page.waitForFunction(()=>document.querySelector('#status').textContent.startsWith('Salvo no projeto.'));
    assert.equal(JSON.parse(fs.readFileSync(path.join(config.root,'data/key-items.json'))).hoenn[0].category,before.hoenn[0].category);
    assert.equal(fs.readdirSync(path.join(config.root,'backups')).length,2);
    await page.setViewportSize({width:1440,height:1000});
    reports.push('comparacao, cancelar sem gravar, restaurar rascunho, desfazer e backup da versao substituida');
    ...
    await page.locator('#document').selectOption('machines.json');
    await page.waitForFunction(()=>document.querySelector('#status').textContent.includes('Prévia atualizada'));
    assert.equal(await page.locator('#dirty').innerText(),'Sem alterações');
    assert.equal(await warnsOnExit(),true);
    reports.push('aviso de saida considera rascunhos em outros documentos');
    ...
    await page.getByText('HTML avançado',{exact:true}).click();
    await page.getByLabel('HTML do conteúdo',{exact:true}).fill('<p>Rascunho imediato</p>');
    assert.equal(await warnsOnExit(),true);
    const stored=await page.evaluate(()=>Object.entries(localStorage).filter(([key])=>key.includes('wiki-editor-draft:')&&key.endsWith(':guides.json')).map(([,v])=>JSON.parse(v)));
    assert.equal(stored[0].data.safari.content,'<p>Rascunho imediato</p>');
    await page.locator('#undo').click();
    reports.push('HTML avancado preservado antes de perder foco');
