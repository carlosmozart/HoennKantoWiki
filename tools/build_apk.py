...
import subprocess
import sys
from build_android_web import build, ROOT
...
def find_java():
    local=ROOT/".local-editor/java-home.txt"
...
def main():
    ...
    (ROOT/"android/local.properties").write_text("sdk.dir="+sdk.as_posix()+"\n",encoding="utf-8")
    build()
    subprocess.run(["node",str(ROOT/"node_modules/@capacitor/cli/bin/capacitor"),"sync","android"],
    ...
    subprocess.run(gradle,cwd=ROOT/"android",env=env,stdin=subprocess.DEVNULL,check=True)
    source=ROOT/"android/app/build/outputs/apk/debug/app-debug.apk"
    version=json.loads((ROOT/"package.json").read_text())["version"]
    target=ROOT/"dist"/f"HoennKantoWiki-{version}-debug.apk"
