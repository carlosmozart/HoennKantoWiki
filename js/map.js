    ...
    renderEncounters(groupedEncounters, container) {
        ...
        for (let method in groupedEncounters) {
            ...
            pokes.forEach(p => {
                ...
                html += `
                    <div class="bento-item" style="padding:10px; text-align:center; cursor:pointer;" onclick="playClickSound(); window.location.hash='pokemon/${p.id}'" title="Ver Dex">
                        <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${p.id}.png" style="width:70px; height:70px; filter:drop-shadow(2px 2px 3px rgba(0,0,0,0.4));">
